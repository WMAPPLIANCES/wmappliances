...
class WmAppSupabaseUser extends BaseAuthUser {
  WmAppSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> wmAppSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = WmAppSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
